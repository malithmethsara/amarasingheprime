...
(function() {
    ...
    // New function: Starts the clock and updates every second
    function startLiveClock() {
        const timeEl = getElementSafe('timeDateTime');
        if (timeEl) {
            const updateTime = () => {
                // Forces the time to display with Weekday, full date, and HH:mm:ss format
                timeEl.textContent = new Date().toLocaleString('en-LK', {
                    weekday: 'long', // ADDS THE DAY NAME (Sunday, Monday, etc.)
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
                    minute: '2-digit',
                    second: '2-digit',
                    hour12: true // Ensures AM/PM if locale defaults to it
                });
            };
            
            updateTime(); // Run once immediately
            setInterval(updateTime, 1000); // Run every 1000ms (1 second)
        }
    }
...
                ['Vehicle Entitlement Levy', formatNumber(resultData.vel), ((resultData.vel/data.totalTax)*100).toFixed(1) + '%'],
                ['VAT (18%)', formatNumber(data.vat), ((data.vat/data.totalTax)*100).toFixed(1) + '%'],
                ['TOTAL TAXES', formatNumber(data.totalTax), '100.0%']
            ],
            theme: 'grid',
            styles: { fontSize: 8, cellPadding: 2, lineWidth: 0.2 },
            headStyles: { fillColor: [0, 48, 135], textColor: [255, 255, 255] },
