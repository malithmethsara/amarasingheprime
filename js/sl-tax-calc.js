...
(function() {
    ...
    function initStickyHeader() {
        const header = getElementSafe('mainHeader');
        if (!header) return;
        let ticking = false;
        const onScroll = () => {
            if (!ticking) {
    ...
    function displayResults(data) {
        ...
        const html = `
            <div class="result-subtitle">Inputs Summary</div>
            <table class="result-table">
                <thead>
                    <tr><th class="col-desc-half">Input Description</th><th class="td-right col-val">Value</th></tr>
                </thead>
                <tbody>
                    <tr><td>CIF (JPY)</td><td class="td-right">${formatNumber(resultData.cifJPY)}</td></tr>
                    <tr><td>Exchange Rate</td><td class="td-right">${resultData.exchangeRate.toFixed(4)}</td></tr>
                    <tr><td>CIF (LKR)</td><td class="td-right">${formatNumber(data.cif)}</td></tr>
                    <tr><td>Vehicle Type</td><td class="td-right">${typeText}</td></tr>
                    <tr><td>Capacity</td><td class="td-right">${formatNumber(resultData.capacity)} ${unit}</td></tr>
                    <tr><td>Age</td><td class="td-right">${ageText}</td></tr>
                </tbody>
            </table>

            <div class="result-subtitle mt">Tax Breakdown (CUSDEC Format)</div>
            <table class="result-table">
                <thead>
                    <tr>
                        <th class="col-desc">Tax Type</th>
                        <th class="td-right col-val">Amount (LKR)</th>
                    </tr>
                </thead>
                <tbody>
                    <tr><td>Customs Duty (CID - 30%)</td><td class="td-right">${formatNumber(data.cid)}</td></tr>
                    <tr><td>Surcharge (50% of CID)</td><td class="td-right">${formatNumber(data.surcharge)}</td></tr>
                    <tr><td>Excise Duty (XID)</td><td class="td-right">${formatNumber(data.excise)}</td></tr>
                    <tr><td>Social Security Levy (SSL - 2.5%)</td><td class="td-right">${formatNumber(data.sscl)}</td></tr>
                    <tr><td>VAT (18%)</td><td class="td-right">${formatNumber(data.vat)}</td></tr>
                    <tr><td>Luxury Tax</td><td class="td-right">${formatNumber(data.luxuryTax)}</td></tr>
                    <tr><td>Vehicle Entitlement Levy</td><td class="td-right">${formatNumber(data.vel)}</td></tr>
                    <tr><td>COM / Exam / Seal Fee</td><td class="td-right">${formatNumber(data.comFee)}</td></tr>
                </tbody>
                <tfoot>
                    <tr><td>Total Taxes & Duties</td><td class="td-right">${formatNumber(data.totalTax)}</td></tr>
                </tfoot>
            </table>

            <div class="result-subtitle mt-lg">Final Cost Summary</div>
            <table class="result-table tfoot-highlight">
                <thead>
                    <tr>
                        <th class="col-desc">Cost Component</th>
                        <th class="td-right col-val">Amount (LKR)</th>
                    </tr>
                </thead>
                <tbody>
                    <tr><td>Vehicle CIF Value</td><td class="td-right">${formatNumber(data.cif)}</td></tr>
                    <tr><td>Total Taxes & Duties</td><td class="td-right">${formatNumber(data.totalTax)}</td></tr>
                    <tr><td>Other Charges (Bank, Dealer & Clearing Fee)</td><td class="td-right">${formatNumber(data.otherCharges)}</td></tr>
                </tbody>
                <tfoot>
                    <tr><td>TOTAL IMPORT COST</td><td class="td-right">${formatNumber(data.totalCost)}</td></tr>
                </tfoot>
            </table>
        `;
        const resultEl = getElementSafe('result');
        ...
        vehicleInventory.forEach(car => {
            ...
            <div class="vehicle-card">
                <div class="veh-img-container">
                    <img src="vehicle-photos/${car.image}" alt="${car.model}" loading="lazy" decoding="async">
                    ${badgeHtml}
                </div>
                <div class="veh-info">
                    <h3>${car.model}</h3>
                    ...
                    <div class="veh-specs-grid">
                        ...
                        <div class="spec-item" title="Auction Grade"><span class="spec-icon">\u{1F4CA}</span> Grade ${car.specs.auction}</div>
                    </div>
                    ...
                    <a href="https://wa.me/94769447740?text=Hi! I would like to get a quotation for the ${encodeURIComponent(car.model)}." target="_blank" rel="noopener" style="text-decoration: none;">
                        <button class="veh-btn">
                            Get Quotation <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 448 512" fill="currentColor"><path d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-26.5l-6.7-4.2-69.8 18.3 18.6-68.1-4.4-6.9c-18.3-29.1-28-63.1-28-97.6 0-101.9 82.9-184.8 184.8-184.8 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7 .9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z"/></svg>
                        </button>
                    </a>
                </div>
            </div>
    ...
    function init() {
        ...
        const leftArrow = document.querySelector('.google-review-section .left-arrow');
        const rightArrow = document.querySelector('.google-review-section .right-arrow');
